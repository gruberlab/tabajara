...
hmm-prospector.pl -d all_reads.fastq -i vFam-A_2014.hmm -s 10 -a -a /usr/local/genome/databases/vfam/annotationFiles_2014/
...
my $transeq_name;
#Verify if the hmmsearch file exists
...
if(-e $input_file){
	...
	if(!defined $hmm_db){
        ...
        my $type = verificaArquivoFasta($input_file);
	...
	#verifica a existência do arquivo tabular
	...
	my $type = verificaArquivoTabular($input_file);	
...
=pod
    if($taxon_tag > 0){
	print TBL1 "Target Name\tquery_pHMM\tE-value\tScore\tTaxon\tProtein\tRange\tType\n";
    }
    else{
...
=pod
    if($taxon_tag > 0){
	print TBL2 "HMM\t# of seqs\tTaxon\tProtein\tRange\tType\n";
    }
    else{
    	print TBL2 "HMM\t# of seqs\n";
    }
